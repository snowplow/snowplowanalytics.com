...
In a relational database, the nature of these entities and the relationships between them are implicit. A user with knowledge of the domain and the particular data model will be able to say what role each entity plays in the event and how the entities are related to each other. However, in a graph, relationships **must** be explicit. And, because relationships in graph databases are **always** directed, the roles of the different entities must also be explicit.

In an RDBMS, a subject and an object will be related by virtue of belonging to the same row (event). It is up to the user to decide which entity plays what role. But in a graph, the subject and the object of the event will have to be explicitly related by a directed relationship that clearly indicates what role each entity plays.

In order to construct a graph, we must choose how to represent events as nodes and edges. To do that, it's necessary to interpret the events. A pared-down interpretation of a `page_view` event in the framework of our event grammar might look something like this:
