...
2. Where there were genuine issues, it was hard to aggregate over the bad row data to size those issues
  1. Figure out what type of data caused the issue (e.g. for a schema validation error, what schema the issue was with, and what field the issue was with), and
  2. Identify the source of the error so that it could be quickly fixed e.g. is it an issue with a specific mobile app?
