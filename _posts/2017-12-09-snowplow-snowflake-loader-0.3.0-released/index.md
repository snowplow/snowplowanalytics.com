...
Most of infranstructure will be handled by embedded `setup` subcommand.
The only one entity you'll need to create by yourself is Snowflake database, all other auxiliary entities, such as warehouse and stage will be created by `setup`.
...
Notice please that `accessKeyId` and `secretAccessKey` are optional. You can include them when running `setup`, but omit later - this could be useful if you want to store your credentials inside Snowflake stage object
...
And last config you'll need to have is Dataflow Runner playbook responsible for running Transformer and Loader. Here's an example:
...
From infranstructure point of view you'll need to:

* Setup DynamoDB table (`manifest` property from Snowflake config) with `RunId` primary key. It'll be used by Transformer and Loader to store state and communicate with each other.
* Run `./Snowplow-snowflake-loader-0.3.0.jar setup --config /path/to/config.json --resolver /path/to/resolver.json`. It'll setup all necessary Snowflake entities according to configuration.
