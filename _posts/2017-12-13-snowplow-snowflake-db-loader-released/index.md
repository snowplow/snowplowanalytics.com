...
Over the last few years Amazon Redshift became a standard de-facto solution used across most of the Snowplow userbase. The simplicity of setting up Redshift, along with its ability to scale to handle large data volumes, and widely supported PostgreSQL API have made it the number one data warehousing location for Snowplow data.
...
Snowflake is relatively new data warehousing technology, quickly gaining popularity. Snowflake has three features that distinguish it from Redshift in particular:
...
Snowflake DB scales fast. In contrast, resizing a Redshift cluster takes hours and can take days - periods during which you can no longer load data into the data warehouse. Whilst this is fine for many users, for high volume users it can create a backlog of data that needs to be loaded.
...
The Snowplow Snowflake Loader, very much like RDB Loader, consists of two parts, both residing at the same repository:
...
*  Currently, the Snowflake Loader can be run in parallel with RDB loader so you can load Snowflake alongside of Redshift. This is useful if you want to evaluate one against the other.
* Both loaders work with enriched data, produced by the Snowplow Spark (or Hadoop) Enrich job.
* The Enrich job and RDB Shredder/Loader can be run directly using EmrEtlRunner. However, the recommended way of running the Snowflake Transformer and Loader is to use [Dataflow Runner][dataflow-runner], which becoming our default orchestration tool across all Snowplow pipelines as per [this RFC][dataflow-runner-rfc].
...
* [More robust manifests][issue-manifest]. Currently the Snowflake Shredder and Loader use separate DynamoDB manifests, which is extremely useful and proven to be reliable, but still can be made more efficient
