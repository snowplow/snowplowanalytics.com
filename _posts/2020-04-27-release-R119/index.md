...
title: "Snowplow R119 production release with new bad row format"
tags: [snowplow, enrichment, bad-rows, release]
author: MikeN
category: Releases
permalink: /blog/2020/04/27/snowplow-release-r119/
---

We are excited to release [Snowplow R119 Tycho Magnetic Anomaly Two][snowplow-release], named after the
[powerful Space Odyssey monolithic artefact that breaks itself into smaller objects](https://2001.fandom.com/wiki/Jovian_Monolith).

This Snowplow release includes the production ready version of the new failed event (bad row) format. We have updated the microservices that make up the Snowplow pipeline to emit much more highly structured bad rows, that should be significantly easier to query and debug. We orignally announced this in the beta version release [Snowplow R118][previous-release].

This significant release allows Snowplow users to have a much better experience when investigating the cause of failed events. With this version of the pipeline failed events are still stored in S3 or Google Cloud Storage, but are now partitioned by type. For more information on the different types of failed events see our documentation [here][failed-events-docs].

Something, in addition, this release also includes important refactoring in the libraries that we use (e.g. `cats` instead of `scalaz` and `circe` instead of `json4s`),
...
we are no longer adding new features to Spark from this release. This release, and all subsequent releases, will focus on our streaming solution.
...
<h1 id="betarelease">1. Production release</h1>

This release represents a major upgrade of the core Snowplow technology. After running this version through it's paces on several QA pipelines as well as with some customer pipelines, we are excited to publish the production ready release.
...
1. It was hard to distinguish “bad rows” that mattered i.e. real data that was not successfully processed, with rubbish data e.g. generated by bots crawling the collector endpoint
...
In this scenario, the payload can’t be stored in its entirety, and a size violation bad row is emitted. For example this could be the case if a big image is sent via a form.
...
We support [many webhooks](https://github.com/snowplow/snowplow/wiki/Setting-up-a-webhook) that can be connected to the Snowplow pipeline. Whenever the event sent by a webhook doesn’t respect the expected structure and list of fields for this webhook, an adapter failure is emitted.
...
The Snowplow pipeline can receive
...
This could happen for instance if the credentials used for the SQL enrichment are wrong, or because of a bad link to the MaxMind database for IP lookup.
...
Scala library [snowplow-badrows](https://github.com/snowplow-incubator/snowplow-badrows) contains all the case classes defining the new bad rows and makes it easier to work with them. By using the same version of the library that the application producing bad rows, we can be sure that there won’t be any incompatibility when processing them.
...
[previous-release]: https://snowplowanalytics.com/blog/2020/01/16/snowplow-release-r118-badrows/
