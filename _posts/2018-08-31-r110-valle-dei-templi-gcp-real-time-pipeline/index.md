...
real-time enrichment process on Google Cloud Platform to [Dataflow][dataflow].
...
Beam Enrich is our new enrichment platform targeting Google Cloud Platform's [Dataflow][dataflow].
It effectively replaces Stream Enrich on GCP which we announced in [Release 101][r101].

This move is part of our strategy to go GCP-native which we outlined in
[our "Porting Snowplow to Google Cloud Platform" RFC][rfc].

At a high level, it works in the same way as the existing enrichment platforms: it takes as input a
[Google Cloud PubSub][pubsub] topic of raw data collected by the Scala Stream Collector, enrich it,
and outputs both successfully enriched events to a topic as well as a stream of events that have
failed enrichment to another.

There are a multitude of reasons why Dataflow running Beam Enrich is better than compute instances
...
- A great programming model: you can interact with Dataflow through the [Apache Beam][beam] API
which __really__ provides the same programming model for batch and streaming computations as well
as abstracts over the execution engine (which can be Dataflow, [Spark][spark] or [Flink][flink]).
Additionally to this programming model, Spotify has built a very nice Scala API on top of Beam
called [Scio][scio] which Beam Enrich leverages.

<h2 id="ssc">2. Bugfixes</h2>
...
When leveraging the PII enrichment introduced in [release 106][r106], the `contexts` field in the
...
`iglu:com.snowplowanalytics.snowplow/parent_event/jsonschema/1-0-0`. This has been fixed with
this release.
...
The Docker image for Beam Enrich is also published on [our Bintray][beam-docker-dl].
...
discussed above can be found on our Bintray [here][se-dl].
